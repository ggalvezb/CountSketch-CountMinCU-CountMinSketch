# C:\Users\ggalv\Google Drive\Respaldo\Magister\Big Data\Tarea 1			python countmin_cu.py
...
def Countmincu(line):
	for i in range(len(line)):
		...
		for j in range(d):
			...
			sketch_temp.append(hashval)
		hashval=min(sketch_temp)
		sketch[j][hashval]+=1
	return(sketch,lista_item)

def RecuentoCountmincu(lista_item,recuento):
	...
	for i in range(len(lista_item)):
		...
		recuento_temp=int(sum(recuento_temp)/d)
...
for line in open(dataset):
	line=line.split()
	del line[0]
	sketch,lista_item=Countmincu(line)




	# #Llevo control para detener la wea
	# i+=1
	# if i==30:
	# 	break

#Genero el recuento de los items
recuento=RecuentoCountmincu(lista_item,recuento)
# print(recuento)

#Transformo la salida a un excel
workbook = xlsxwriter.Workbook('Frecuencia Countmin CU.xlsx')
